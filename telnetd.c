...
// Modify-SHG
#define MAXSESSION 3
#define SESSION_ALIVE_TIME_SEC 600
int gSessionCount = 0;
...
make_new_session(
		...
		IF_FEATURE_TELNETD_STANDALONE(, struct sockaddr_in *paddr)
) {
#if !ENABLE_FEATURE_TELNETD_STANDALONE
	enum { sock = 0 };
#endif
	const char *login_argv[4]; // login -h ip주소
	...
	if ( paddr ) {
		snprintf(straddr, sizeof(straddr), "%s", inet_ntoa(paddr->sin_addr));
		//printf("%s:%d] remote source addr[0x%x][%s]\n", __func__, __LINE__, paddr->sin_addr.s_addr, straddr);
		login_argv[1] = "-h";
		login_argv[2] = straddr;
		login_argv[3] = NULL;
	}
	else {
		login_argv[1] = NULL;
...
*/
 again:
	...
	if (!IS_INETD && FD_ISSET(master_fd, &rdfdset)) {
		...
		if( MAXSESSION > gSessionCount ) {
			/* Create a new session and link it into active list */
			new_ts = make_new_session(fd, NULL);
			if (new_ts) {
				new_ts->next = G.sessions;
				new_ts->time = (int)time(NULL); //세션 등록된 시간을 표시한다.
				G.sessions = new_ts;
				gSessionCount++;
			} else {
				close(fd);
	...
	while (ts) { /* For all sessions... */
		...
		if (/*ts->size1 &&*/ FD_ISSET(ts->ptyfd, &wrfdset)) {
			...
			if( (nTime - ts->time) > SESSION_ALIVE_TIME_SEC ) 
				goto kill_session;
			else
				ts->time = (int)time(NULL);
		}
 skip1:
